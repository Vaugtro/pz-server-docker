...
if [ -n "${SERVERPRESET}" ]; then
  ...
  elif [ ! -f "/app/Server/${SERVERNAME}_SandboxVars.lua" ] || [ "${SERVERPRESETREPLACE,,}" == "true" ]; then
    echo "*** INFO: New server will be created using the preset ${SERVERPRESET} ***"
    echo "*** Copying preset file from /app/media/lua/shared/Sandbox/${SERVERPRESET}.lua\" to \"/app/Server/${SERVERNAME}_SandboxVars.lua\" ***"
    mkdir -p "/app/Server/"
    cp -nf "/app/media/lua/shared/Sandbox/${SERVERPRESET}.lua" "/app/Server/${SERVERNAME}_SandboxVars.lua"
    sed -i "1s/return.*/SandboxVars = \{/" "/app/Server/${SERVERNAME}_SandboxVars.lua"
    # Remove carriage return
    dos2unix "/app/Server/${SERVERNAME}_SandboxVars.lua"
    # I have seen that the file is created in execution mode (755). Change the file mode for security reasons.
    chmod 644 "/app/Server/${SERVERNAME}_SandboxVars.lua"
  fi
fi
...
if [ -n "${PASSWORD}" ]; then
	sed -i "s/Password=.*/Password=${PASSWORD}/" "/app/Server/${SERVERNAME}.ini"
fi

if [ -n "${MOD_IDS}" ]; then
 	echo "*** INFO: Found Mods including ${MOD_IDS} ***"
	sed -i "s/Mods=.*/Mods=${MOD_IDS}/" "/app/Server/${SERVERNAME}.ini"
fi

if [ -n "${WORKSHOP_IDS}" ]; then
 	echo "*** INFO: Found Workshop IDs including ${WORKSHOP_IDS} ***"
	sed -i "s/WorkshopItems=.*/WorkshopItems=${WORKSHOP_IDS}/" "/app/Server/${SERVERNAME}.ini"
...
if [ -e "/app/steamapps/workshop/content/108600" ]; then
  ...
  if [ -n "${map_list}" ]; then
    ...
    sed -i "s/Map=.*/Map=${map_list}Muldraugh, KY/" "/app/Server/${SERVERNAME}.ini"
    ...
    for string in "${strings[@]}"; do
        if ! grep -q "$string" "/app/Server/${SERVERNAME}_spawnregions.lua"; then
          if [ -e "/app/media/maps/$string/spawnpoints.lua" ]; then
            ...
            \\\t\t$result
            }" "/app/Server/${SERVERNAME}_spawnregions.lua"
          fi
        fi
    done
  fi 
